...
namespace Elegance.AspNet.Authentication.Extensions
{
	public static class ClaimsPrincipalExtensions
	{
		extension(ClaimsPrincipal @this)
		{
			...
			[PublicAPI]
			public bool TryGetClaimValue(string claimType, [NotNullWhen(true)] out string? result)
			{
			...
			public bool TryGetClaimValue<T>(string claimType, [NotNullWhen(true)] out T? result) where T : System.ISpanParsable<T>
			{
			...
			[PublicAPI]
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public T? GetClaimValue<T>(string claimType) where T : System.ISpanParsable<T> =>
			...
			public string GetRequiredClaimValue(string claimType) =>
				@this.TryGetClaimValue(claimType, out var result) ? result : throw new Exception("Claim type not found: " + claimType);
		}
	}
}
